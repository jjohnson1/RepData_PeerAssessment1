...
`r sprintf("%g", meanSteps)` and the median is `r medianSteps`.
...
`r maxInterval`. This is also evidenced in the chart.
...
This tells us that there are `r nMissingPoints` observations with missing values.
...
number of steps for the interval, averaged over all days in the data set.
...
The new mean number of steps per day is `r sprintf("%g", imputedMeanSteps)` 
and the median is `r sprintf("%g", imputedMedianSteps)`. The mean has not changed,
